...
const cert = require('./safekey.json');
...
async function run() {
  try {
    const businessesSnap = await db.collection('businesses_public').limit(500).get();
    ...
    for (const data of users) {
      ...
      // Exact VIP check from UI: data.isVip === true || data.subscription?.plan === 'vip' || data.selectedPlan === 'vip'
