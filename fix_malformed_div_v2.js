...
// The issue was caused by my previous regex grabbing everything up to line 467 but leaving the rest of the onClick handler orphaned.
// The buttononClick was sliced in half!

// The safest way is to rebuild exactly what broke.

const startMarker = `                        {isMapReady && (`;
const endMarker = `            {/* Map with draggable marker */}`;

const startIndex = content.indexOf(startMarker);
const endIndex = content.indexOf(endMarker, startIndex);

if(startIndex !== -1 && endIndex !== -1) {
    const brokenChunk = content.substring(startIndex, endIndex);

    const correctChunk = `        {isMapReady && (
...
`;
    // We replace the broken block safely
    content = content.replace(brokenChunk, correctChunk);
    fs.writeFileSync(path, content, 'utf8');
    console.log("Rewrote missing JS perfectly.");
} else {
