...
export default function AdminUsersPage() {
    ...
    const FILTERS: { key: Filter; label: string; count: number }[] = [
        ...
        { key: 'providers', label: 'Proveedores', count: providers },
        { key: 'clients', label: 'Clientes', count: clients },
        { key: 'new', label: 'Nuevos 30d', count: newLast30 },
        { key: 'banned', label: 'Bloqueados', count: banned },
    ];
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                        <Users size={24} className="text-brand-neon-cyan" /> Usuarios
                    </h1>
                    <p className="text-slate-400 text-sm mt-0.5">
                        {users.length} usuarios{selectedCountry !== 'ALL' && ` en ${selectedCountry}`}
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                {[
                    { label: 'Proveedores', val: providers, icon: <Building2 size={16} />, color: 'text-purple-400' },
                    { label: 'Clientes', val: clients, icon: <UserCheck size={16} />, color: 'text-blue-400' },
                    { label: 'Nuevos (30d)', val: newLast30, icon: <Clock size={16} />, color: 'text-green-400' },
                    { label: 'Bloqueados', val: banned, icon: <UserX size={16} />, color: 'text-red-400' },
                ].map(s => (
