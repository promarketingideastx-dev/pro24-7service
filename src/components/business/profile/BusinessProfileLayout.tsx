...
export default function BusinessProfileLayout({ business, activeTab, onTabChange, children, isOwner, onBookClick, isModalOpen }: ProfileLayoutProps) {
    ...
    return (
        <main className="min-h-screen bg-[#0f0d1a] text-white font-sans pb-24 md:pb-20">
            ...
            <header className="relative w-full">
                ...
                <div className="h-48 md:h-64 lg:h-72 w-full bg-slate-800 relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0f0d1a] via-[#0f0d1a]/60 to-transparent"></div>
                </div>
                ...
                <div className="px-6 -mt-12 relative flex flex-col md:flex-row gap-4 md:items-end">
                    ...
                    {/* Avatar / Logo */}
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-2xl md:rounded-3xl bg-slate-800 border-4 border-[#0f0d1a] shadow-2xl overflow-hidden shrink-0 relative z-10 group">
                        ...
                        {business.verified && (
                            <div className="absolute bottom-1 right-1 bg-green-500 text-white rounded-full p-0.5 border-2 border-[#0f0d1a]">
            ...
            {/* --- 2. STICKY TABS --- */}
            <div className={`sticky top-0 z-40 bg-[#0f0d1a]/80 backdrop-blur-xl border-b border-white/5 mt-6 transition-all duration-300 ${isSticky ? 'shadow-lg shadow-black/50' : ''}`}>
            ...
            {/* --- 3. CONTENT AREA --- */}
            <div className="min-h-[50vh] bg-[#0f0d1a]">
                {children}
            </div>

            {/* --- 4. MOBILE STICKY ACTION BAR (hidden when modal is open) --- */}
            {!isOwner && !isModalOpen && (
                <div className="fixed bottom-0 left-0 right-0 bg-[#0f0d1a]/90 backdrop-blur-lg border-t border-white/10 p-4 md:hidden z-50 flex gap-3 animate-in slide-in-from-bottom-full duration-500">
                    {/* WhatsApp Button */}
                    <button
