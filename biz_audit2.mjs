...
async function runAudit() {
  ...
  const payloadReq = { structuredQuery: { from: [{ collectionId: 'businesses' }], limit: 10 } };
  ...
  bizList.filter(r => r.document).forEach(r => {
      console.log("Found BIZ:", r.document.name.split('/').pop());
      console.log(Object.keys(r.document.fields));
      if (r.document.fields.country) console.log("Country structure:", JSON.stringify(r.document.fields.country));
      if (r.document.fields.paymentSettings) console.log("Payment Settings:", JSON.stringify(r.document.fields.paymentSettings));
  });
}
