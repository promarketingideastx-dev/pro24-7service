...
async function runAudit() {
  ...
  const creds = { project_id: projectIdMatch[1], client_email: clientEmailMatch[1], private_key: privateKeyMatch[1].replace(/\\n/g, '\n') };

  const header = { alg: 'RS256', typ: 'JWT' };
  const now = Math.floor(Date.now() / 1000);
  const payload = { iss: creds.client_email, sub: creds.client_email, scope: "https://www.googleapis.com/auth/datastore", aud: 'https://oauth2.googleapis.com/token', iat: now, exp: now + 3600 };
  ...
  const res = await fetch(`https://firestore.googleapis.com/v1/projects/${creds.project_id}/databases/(default)/documents/businesses/PRyuyfXzLAMjLhhaPSbI7vZWAwt1`, { method: 'GET', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }});
